...
from src.special_swing import SPECIAL_SWING_AUDIT_COLUMNS, SPECIAL_SWING_COLUMNS
...
def save_special_swing_candidates(
    ...
    path = result_dir / f"{signal_date}_special_swing_candidates_top{candidate_count}.csv"
    normalized_df = ensure_columns(candidates_df, SPECIAL_SWING_COLUMNS)
    normalized_df[SPECIAL_SWING_COLUMNS].to_csv(path, index=False, encoding=CSV_ENCODING)
    return path
...
def save_special_swing_all_evaluated(
    ...
    result_dir.mkdir(parents=True, exist_ok=True)
    path = result_dir / f"{signal_date}_special_swing_all_evaluated.csv"
    normalized_df = ensure_columns(evaluated_df, SPECIAL_SWING_AUDIT_COLUMNS)
    normalized_df[SPECIAL_SWING_AUDIT_COLUMNS].to_csv(path, index=False, encoding=CSV_ENCODING)
    return path
...
def save_special_swing_news_markdown(
    ...
    result_dir.mkdir(parents=True, exist_ok=True)
    path = result_dir / f"{signal_date}_special_swing_news_raw_top{candidate_count}.md"
...
def build_special_swing_news_markdown(
    ...
    lines = [
        f"# Special Swing Raw News - {signal_date}",
        "",
...
def save_special_swing_news_dataset(
    ...
    result_dir.mkdir(parents=True, exist_ok=True)
    path = result_dir / f"{signal_date}_special_swing_news_dataset_top{candidate_count}.json"
...
def build_special_swing_news_dataset(
    ...
    return {
        ...
        "task": f"Codex should score Top{candidate_count} candidates, select Top{shortlist_n}, then run a four-specialist and one-leader debate to select final Top{final_n}.",
