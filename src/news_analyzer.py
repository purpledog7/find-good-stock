...
def enrich_news_info(
    ...
    for index, row in result.iterrows():
        name = str(row["name"])
        emit_progress(progress, f"뉴스 검색 중 ({len(rows) + 1}/{len(result)}): {name}")
        try:
        ...
        rows.append(summarize_news_items(items))

    news_df = pd.DataFrame(rows, index=result.index)
    return pd.concat([result, news_df], axis=1)
